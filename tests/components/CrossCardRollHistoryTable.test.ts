...
describe('CrossCardRollHistoryTable', () => {
  it('複数カードの出目履歴を新しい順に表示する', () => {
    ...
    const wrapper = mount(CrossCardRollHistoryTable, { props: { cards } })
  ...
  it('列は日時と出目のみ', () => {
    ...
    const wrapper = mount(CrossCardRollHistoryTable, { props: { cards } })
    expect(wrapper.findAll('thead th').map((th) => th.text())).toEqual(['日時', '出目'])
  ...
  it('初期表示は指定件数までに抑え、ボタンで残りを表示する', async () => {
    ...
    const wrapper = mount(CrossCardRollHistoryTable, { props: { cards, initialLimit: 2 } })
  ...
  it('件数が上限以下なら展開ボタンを出さない', () => {
    const cards = [makeCard({ rolls: [makeRoll(1, 100)] })]
    const wrapper = mount(CrossCardRollHistoryTable, { props: { cards, initialLimit: 30 } })
    expect(wrapper.find('.history-more').exists()).toBe(false)
  })
})
