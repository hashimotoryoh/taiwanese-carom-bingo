...
- グラフ（`RollAverageTrendChart`）はライブラリを使わずインライン SVG で描く。表示レンジやラベル間引きの計算は `app/utils/chart.ts` に切り出してある。横の目安線はデータによらず常に 0・40・80 の3本で、表示レンジにも必ず含める。SVG の `viewBox` は `useElementWidth`（`ResizeObserver`）で測った実寸に合わせており、1 単位 = 1 CSS ピクセルになるため文字サイズは CSS の `px` 指定がそのまま効く。
