...
サイコロは120面体（出目 $v$ は $1 \sim 120$ の一様分布）。出目に応じた得点 $f(v)$ は以下のとおり:

- **ゾロ目**（2桁以上で全桁同一数字）: $f(v) = -v$
- **$v = 100$**: $f(100) = 200$
- **$v = 101$**: $f(101) = 101 + (\text{もう一回振った出目の得点})$
- **その他**: $f(v) = v$
...
$$E = \frac{1}{120}\sum_{v=1}^{120} f(v)$$

まず「全ての目が $f(v) = v$」だった場合の基準値を計算する:

$$\sum_{v=1}^{120} v = \frac{120 \times 121}{2} = 7260$$

**ゾロ目の補正**: 120以下のゾロ目は $11, 22, 33, 44, 55, 66, 77, 88, 99, 111$ の10個。本来 $+v$ のところが $-v$ になるため、1つにつき $-2v$ の補正が入る。

$$11 + 22 + \cdots + 99 = 11 \times (1 + 2 + \cdots + 9) = 11 \times 45 = 495$$
$$495 + 111 = 606$$
$$\text{補正}_{\text{ゾロ目}} = -2 \times 606 = -1212$$
...
$$\sum_{v=1}^{120} f(v) = 7260 - 1212 + 100 + E = 6148 + E$$
